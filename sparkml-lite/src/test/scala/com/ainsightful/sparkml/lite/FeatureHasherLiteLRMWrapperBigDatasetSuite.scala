...
import org.apache.spark.ml.classification.{LRMWrapper, LogisticRegression, LogisticRegressionModel}
...
class FeatureHasherLiteLRMWrapperBigDatasetSuite extends FunSuite with MockitoSugar with BeforeAndAfter {
  ...
  private def predictWithFeatureHasher(hashSize: Int,
    ...
    val predictions = df3.select("probability").rdd.map(r => r(0)).collect().map(x => x.asInstanceOf[DenseVector](1))
    (model, predictions)
  }
  ...
  private def predictWithFeatureHasherLite(model: LogisticRegressionModel, hashSize: Int): Array[Double] = {
    ...
    val wrapperModel = new LRMWrapper(model)
    val predictions =
      lines.map(line => {
